...
Architecture issues #28–#37 are complete. Epic #5 closes after stakeholder review confirms the consolidated baseline and names the launch-time owners/approvals listed in `docs/architecture.md`.
...
| Platform | [#63](https://github.com/ecillie/FBO_Manager/issues/63), early [#64](https://github.com/ecillie/FBO_Manager/issues/64) | Staging has private PostgreSQL, exact-origin HTTPS, secret injection, an environment OIDC client, first/second administrators, and basic health/log visibility. |

**Exit demonstration:** A named MFA-protected staging user signs in, performs an authorized airport/reference update through the browser, sees the committed result after refresh, and produces correlated request and audit evidence. A viewer receives `403` and cannot expose tokens or grant authority.
...
**Exit demonstration:** Administration, aircraft turnaround, service/task dispatch, fuel transfer/dispense/reconciliation, workforce scheduling, and current-state dashboard flows run end to end in staging, including conflicts, unknown outcomes, authorization boundaries, and history retention.
...
| Delivery/operations | [#62](https://github.com/ecillie/FBO_Manager/issues/62), [#63](https://github.com/ecillie/FBO_Manager/issues/63), [#64](https://github.com/ecillie/FBO_Manager/issues/64), [#65](https://github.com/ecillie/FBO_Manager/issues/65) | Signed immutable artifacts promote; staging/production boundaries, alerts, access, backups, and isolated restore are verified. |
...
**Exit demonstration:** The exact signed release candidate passes staging promotion, migration, all required tests/scans, restored-database smoke verification, and the documented observation window with no unaccepted release blocker.
...
4. deployable artifacts, staging identity/network configuration, and baseline operations visibility (#62–#64), followed by the Gate 2 demonstration;
...
- after each merge, synchronize the local branch with the protected default branch, verify the relevant gate demonstration, update the issue/epic checklists, and then select the next dependency-ready slice;
...
11. the artifact, staging, and observability slices from [#62](https://github.com/ecillie/FBO_Manager/issues/62), [#63](https://github.com/ecillie/FBO_Manager/issues/63), and [#64](https://github.com/ecillie/FBO_Manager/issues/64), in that order; then
...
- code, migrations, generated artifacts, and configuration are reviewed and merged;
...
| Platform work starts after feature completion | Schedule small #62–#65 slices at gate checkpoints and demonstrate every gate in staging. |
