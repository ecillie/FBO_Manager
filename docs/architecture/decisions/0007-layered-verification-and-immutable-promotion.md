...
- Build-once promotion makes staging evidence relevant to the exact production bytes.
...
- A shared staging environment can queue unrelated acceptance work and is less isolated than per-PR previews.
...
| Direct pushes or merge commits to main | They weaken required-check/review enforcement and make one change harder to trace. Squash merge fits the small team. |
| Rebuild separately in staging and production | Dependency/base-image drift could make production different from the tested candidate. |
...
Changing PostgreSQL parity, required concurrency/security coverage, branch/review protections, build-once promotion, migration safety, or production approval requires architecture/release review and a superseding ADR when it changes the strategy rather than a tool detail.
