...
| [0007: Layered verification and immutable artifact promotion](0007-layered-verification-and-immutable-promotion.md) | Accepted; branch portions amended | PostgreSQL concurrency, API/security contracts, browser behavior, migrations, and supply chain all need proportional verification. | Risk-based test pyramid on PostgreSQL 18, protected PRs, security/supply-chain gates, build once/sign, exact-digest NonProd-to-production promotion, and forward-safe migrations. | Traceable releases and strong rule coverage; more CI/runbook effort and manual production approval are required. | [0008](0008-environment-aligned-branch-promotion.md) for branch, review, artifact-source, and promotion details |
| [0008: Environment-aligned integration and release promotion branches](0008-environment-aligned-branch-promotion.md) | Accepted | One developer needs `FBODev`, a stable versioned candidate, and production-only `FBOProd` without losing build-once promotion. | Ticket PRs target `FBODev`; `Release-1.0.0` owns MVP acceptance; accepted exact digests promote through `FBOProd` and tag `v1.0.0`; solo review transitions when another developer joins. | Clear production boundary and release traceability; explicit PR bases and forward-port discipline are required. | — |

ADR 0008 explicitly amends the branch/review and promotion-source portions of ADR 0007; ADR 0007 remains controlling for the verification, supply-chain, migration, and immutable-artifact requirements it retains. A fully superseding ADR links the replaced record, changes its status to `Superseded`, and populates both records' index entries; accepted history is never deleted or silently rewritten.
